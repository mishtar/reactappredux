...
class PostList extends Component {
    ...
    render(){
        return(
            <div>
                Orden:{' '}
                <Button color="info" className="post-list-button" active={this.state.order===1} onClick={() => this.handleClick(1)} outline><div>Ascendente</div></Button>{' '}
                <Button color="primary" className="post-list-button" active={this.state.order===0} onClick={() => this.handleClick(0)}><div>Descendente</div></Button>{' '}
                {this.state.posts
